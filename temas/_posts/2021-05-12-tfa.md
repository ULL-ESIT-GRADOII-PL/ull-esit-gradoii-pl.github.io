...
Una mejora a esta situación actual es proveer `async` y `await` en Egg.
Quizá podría ser una sintáxis como esta:
...
Vea el Capítulo [Análisis del Contexto]({{site.baseurl}}/temas/analisis-dependiente-del-contexto/) en estos apuntes y lea el capítulo *Symbol Table Structure* y el Chapter 3 del libro de Muchnik:
...
[generating-js]({{site.baseurl}}/practicas/generating-js) haciéndolo lo mas completo posible.
...
Véase la sección [Lua]({{ site.baseurl }}/practicas/infix2evm#lua) en la práctica *Desde Lenguajes de Infijo a EVM usando Nearley.js*
...
- Como prueba de su capacidad de expresión, reescriba su analizador de Egg o el de la práctica [infix2evm]({{site.baseurl}}/practicas/infix2evm) usando su LexerGenerator.
...
do {
  ...
  print(
    ...
    .repo
    .list(
      'ULL-ESIT-PL-2021, Map{private: true, limit: 20}
    )
  ) 
  /*
  The same as executing the command: 
  "gh repo list ULL-ESIT-PL-2021 --private --limit 20"
  */ 
}
```

Para esto debería de familiarizarse con las funciones para la ejecución de programas y manipulación de entrada/salida de procesos  desde Node.JS.
